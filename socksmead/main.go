...
func main() {
	...
	*sConfig = *p_s_config
	sConfig.GetCertificate = func(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
		...
		log.Printf("Generating new cert %+v", hello.ServerName)
...
func serve(conn net.Conn, port int, host string) {
	data := make([]byte, 4096)
	var outConn, inConn net.Conn
	var err error
	var length int
	log.Printf("Serving %+v, %+v, %+v", conn, port, host)
	inFile, outFile := get_record_files(host)
	if port == 443 {
	...
	go io.Copy(outConn, io.TeeReader(inConn, outFile))
	io.Copy(inConn, io.TeeReader(outConn, inFile))
}
